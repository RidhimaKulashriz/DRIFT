...
import { addTelemetryRecord, createEvidenceRecord } from "../db";
...
async function startServer() {
  ...
  app.post("/api/drift/evidence", async (req, res) => {
    ...
    if (!allowedMimeTypes.has(String(body.mimeType).toLowerCase()) || !new RegExp("^[A-Za-z0-9+/=,\\\\s]+$").test(String(body.base64))) return res.status(400).json({ error: "Only supported image/video media and base64 payloads are accepted." });
    const bytes = Buffer.from(String(body.base64).split(",").pop() ?? "", "base64");
    ...
    try {
      ...
      const result = await createEvidenceRecord({ missionId: Number(body.missionId), fileName: String(body.fileName), mimeType: String(body.mimeType), storageKey: stored.key, storageUrl: stored.url, mediaKind: body.mediaKind as "photo" | "video", latitude: typeof body.latitude === "number" ? body.latitude.toFixed(6) : undefined, longitude: typeof body.longitude === "number" ? body.longitude.toFixed(6) : undefined, playbackSeconds: typeof body.playbackSeconds === "number" ? Math.max(0, Math.floor(body.playbackSeconds)) : undefined });
